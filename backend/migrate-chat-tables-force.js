...
(async () => {
  ...
  try {
    console.log('Running forced chat migrations on DB:', dbName);
    ...
    console.log('Forced chat migrations completed.');
  } catch (e) {
    await client.query('ROLLBACK');
    console.error('Forced migration failed:', e.message);
    process.exitCode = 1;
  } finally {
    client.release();
    await pool.end();
